            ...
            <div className="text-center">
              ...
              <div className="bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl p-6 mb-4">
                ...
                <p className="text-5xl font-bold mb-1">
                  {correctCount}/{totalCount} {plural(totalCount, 'question correcte', 'questions correctes')}
                </p>
              ...
              {questionResults.map((result, index) => (
                ...
                >
                  <div className="flex items-start justify-between gap-3 mb-3">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <span className={`text-xs font-bold px-2 py-1 rounded-full ${result.isCorrect ? 'bg-green-200 text-green-800' : 'bg-red-200 text-red-800'}`}>
                        {result.isCorrect ? `+${result.points} pts` : '0 pt'}
                      </span>
                    </div>
                  </div>
                  ...
                  <div className={`text-sm mb-2 ${result.isCorrect ? 'text-green-800' : 'text-red-800'}`}>
                    ...
                    <span>{result.userAnswerText || <span className="italic opacity-70">Aucune réponse</span>}</span>
                  </div>

                  {!result.isCorrect && (
                    <div className="text-sm text-blue-800 bg-blue-50 border border-blue-300 rounded-lg px-3 py-2">
                      <span className="font-medium">✅ Réponse{result.type === 'MULTI_TEXT' ? 's' : ''} attendue{result.type === 'MULTI_TEXT' ? 's' : ''} : </span>
              ...
              {currentQuestion.answers.map((answer) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full p-4 rounded-lg border-2 transition-all ${showCorrect ? 'border-green-500 bg-green-50'
              ...
              <div className="space-y-3">
                {currentQuestion.answers.map((answer) => {
                  ...
                  return (
                    <label
                      key={answer.id}
                      className={`w-full p-4 rounded-lg border-2 transition-all text-left flex items-center gap-3 ${showCorrect ? 'border-green-500 bg-green-50'
                        : showWrong ? 'border-red-500 bg-red-50'
                          : isSelected ? 'border-blue-600 bg-blue-50'
                            : 'border-gray-200 hover:border-blue-300 hover:bg-gray-50'
                  ...
                  {!isCorrect && (
                    <div className="mt-2 p-3 bg-blue-50 border border-blue-300 rounded-lg">
                      <p className="text-sm font-medium text-blue-900 mb-1">
                        Réponses attendues {currentQuestion.strictOrder ? '(dans l\'ordre)' : '(dans n\'importe quel ordre)'} :
                      </p>
                      <ul className="text-sm text-blue-800 space-y-1">
                        {currentQuestion.answers?.map((a, i) => (
                          <li key={i}>{currentQuestion.strictOrder ? `${i + 1}. ` : '• '}{a.text}</li>
                        ))}
                      </ul>
                    </div>
                  )}
