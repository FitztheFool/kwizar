  ...
  return (
    <button
      ...
      className={`relative w-16 h-16 rounded-xl transition-all duration-200 select-none
        ${held
          ? 'ring-4 ring-amber-400 bg-amber-50 shadow-amber-200 shadow-lg scale-105'
      ...
      <svg viewBox="0 0 100 100" className="w-full h-full p-1">
        {dots.map(([cx, cy], i) => (
          <circle key={i} cx={cx} cy={cy} r={9}
            fill={held ? '#d97706' : '#1e293b'} />
        ))}
        {value === 0 && (
          <text x="50" y="60" textAnchor="middle" fontSize="30" fill="#cbd5e1">?</text>
        )}
      </svg>
      {held && (
        <span className="absolute -top-2 -right-2 bg-amber-400 text-white text-[9px] font-black px-1 rounded-full">
          GARDÉ
        </span>
      )}
    </button>
...
export default function YahtzeePage() {
  ...
  useEffect(() => {
    if (!socket || !lobbyId || status !== 'authenticated') return;
    if (joinedRef.current) return;
    joinedRef.current = true;
    const doJoin = () => {
      socket.emit('lobby:join', { lobbyId, userId: me?.id, username: me?.name ?? me?.email ?? 'Joueur' });
      socket.emit('yahtzee:join', { lobbyId });
    };
    if (socket.connected) {
      doJoin();
    } else {
    ...
    socket.on('yahtzee:state', (state: GameState) => {
      setGame(state);
      setRolling(false);
    });
    socket.on('yahtzee:ended', ({ results }: { results: { userId: string; username: string; total: number }[] }) => {
      setResults(results);
    });
  ...
  }, [socket, lobbyId, status]);

  if (status === 'loading' || !me) return (
  ...
  const roll = () => {
    if (!canRoll) return;
    setRolling(true);
    socket?.emit('yahtzee:roll', { lobbyId, userId: myId });
  };
  ...
  const scoreCategory = (cat: string) => {
    if (!canScore) return;
    if (myPlayer?.scoreCard[cat as keyof ScoreCard] !== null) return;
    socket?.emit('yahtzee:score', { lobbyId, userId: myId, category: cat });
  };

  const forceScore = () => {
    if (!isMyTurn || !canScore) return;
    socket?.emit('yahtzee:forceScore', { lobbyId, userId: myId });
  };
      ...
      <div className="min-h-screen bg-slate-900 flex items-center justify-center p-6">
        <div className="bg-slate-800 rounded-2xl p-8 max-w-md w-full shadow-2xl text-center">
          ...
          <div className="space-y-3">
            {results.map((p, i) => (
              <div key={p.userId} className={`flex items-center justify-between px-4 py-3 rounded-xl
                ${i === 0 ? 'bg-amber-400/20 border border-amber-400/50' : 'bg-slate-700/50'}
              `}>
                <div className="flex items-center gap-3">
                  <span className="text-2xl">{i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : `${i+1}.`}</span>
                  <span className={`font-bold ${p.userId === myId ? 'text-amber-300' : 'text-white'}`}>
                    {p.username}{p.userId === myId && ' (moi)'}
                  </span>
                </div>
                <span className={`font-black text-xl ${i === 0 ? 'text-amber-400' : 'text-slate-300'}`}>
                  {p.total} pts
                </span>
              </div>
            ))}
    ...
    <div className="min-h-screen bg-slate-900 text-white">
      ...
      <div className="max-w-6xl mx-auto p-4 grid grid-cols-1 xl:grid-cols-[1fr_380px] gap-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-slate-800 rounded-2xl p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-bold text-slate-300">
                {isMyTurn ? 'Vos dés' : `Dés de ${currentPlayer?.username}`}
              </h2>
              {isMyTurn && (
                <div className="flex items-center gap-2">
            ...
            {isMyTurn && (
              <div className="flex gap-3 justify-center">
                ...
                {canScore && game.phase === 'rolling' && (
                  <button onClick={forceScore}
                    className="px-4 py-3 rounded-xl font-semibold text-sm bg-slate-700 hover:bg-slate-600 text-slate-300 transition-all">
                    Choisir score →
                  </button>
                )}
              </div>
            ...
            {isMyTurn && myPlayer && myPlayer.rollsLeft < 3 && myPlayer.rollsLeft > 0 && (
              <p className="text-center text-xs text-slate-500 mt-3">
                Cliquez sur un dé pour le garder, puis relancez. Ou choisissez directement votre score.
              </p>
            )}
            {isMyTurn && game.phase === 'scoring' && (
              <p className="text-center text-sm text-amber-400 mt-3 font-semibold">
                ✍️ Choisissez une catégorie à scorer ci-dessous
              </p>
            )}
          </div>

          {/* Score table for current player */}
          <div className="bg-slate-800 rounded-2xl p-5">
            ...
            <div className="mb-2">
                  ...
                  return (
                    <div key={cat}
                      ...
                      className={`flex items-center justify-between px-3 py-2 rounded-lg transition-all
                        ${val !== null ? 'opacity-60 cursor-default' : canScore ? 'cursor-pointer hover:bg-indigo-500/20 hover:border-indigo-500/40 border border-transparent' : 'cursor-default border border-transparent'}
                      ...
                      <div className="text-right min-w-[40px]">
                        {val !== null ? (
                          <span className="font-black text-slate-300">{val}</span>
                        ) : isHovered && preview !== null ? (
                          <span className="font-black text-indigo-400">{preview}</span>
                        ) : (
              ...
              {upperNeeded > 0 && (
                <div className="px-3 py-1">
                  <div className="flex justify-between text-xs text-slate-500 mb-1">
                    <span>Progression bonus (+35)</span>
                    <span>{upperTotal}/63</span>
                  </div>
              ...
              {myPlayer?.upperBonus === 35 && (
                <div className="px-3 py-1.5 text-center text-xs text-green-400 font-bold">✅ Bonus +35 débloqué !</div>
              )}
            </div>
        ...
        <div className="space-y-4">
          <div className="bg-slate-800 rounded-2xl p-5">
            ...
            <div className="space-y-3">
              {game.players.map((p, i) => {
                ...
                return (
                  <div key={p.userId} className={`p-4 rounded-xl border transition-all
                    ${isCurrent ? 'border-indigo-500/60 bg-indigo-500/10' : 'border-slate-700 bg-slate-700/30'}
                  `}>
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        {isCurrent && <span className="w-2 h-2 rounded-full bg-indigo-400 animate-pulse" />}
                        <span className={`font-bold ${isMe ? 'text-indigo-300' : 'text-white'}`}>
                          {p.username}{isMe && ' (moi)'}
                        </span>
                      </div>
          ...
          {/* Other players' score cards (collapsed) */}
