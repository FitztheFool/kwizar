...
const QUIZ_NAV_ITEMS: { tab?: TabType; label: string; icon: string; href?: string }[] = [
    ...
    { tab: 'quiz-score', label: 'Mes scores', icon: '🏅' },
    { label: '', icon: '', href: '' },
...
export default function Sidebar({ activeTab, onTabChange, isOpen, onClose, userRole, userName, userEmail }: SidebarProps) {
    ...
    return (
        ...
        `}>
            ...
            <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
                ...
                <div>
                    <button
                        onClick={() => { if (collapsed) setCollapsed(false); else setUnoMenuOpen(prev => !prev); }}
                        title="Uno"
                        className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors text-left
                            ${unoSectionActive ? 'bg-orange-50 text-orange-700' : 'text-gray-600 hover:bg-gray-100 hover:text-gray-900'}`}>
                        <span className="text-base flex-shrink-0">🎴</span>
                        {!collapsed && (
                            <>
                                <span>Uno</span>
                                <span className={`ml-auto text-xs transition-transform duration-200 ${unoMenuOpen ? 'rotate-180' : ''}`}>▾</span>
                            </>
                        )}
                    </button>

                    {!collapsed && unoMenuOpen && (
                        <div className="ml-3 mt-1 space-y-0.5 border-l-2 border-gray-100 pl-3">
                            <button onClick={() => handleTab('uno-score')}
                                className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors text-left
                                    ${activeTab === 'uno-score' ? 'bg-orange-50 text-orange-700' : 'text-gray-500 hover:bg-gray-100 hover:text-gray-900'}`}>
                                <span className="text-sm">🏆</span>
                                Mes scores
                                {activeTab === 'uno-score' && <span className="ml-auto w-1.5 h-1.5 rounded-full bg-orange-500" />}
                            </button>
                        </div>
                    )}
                </div>
