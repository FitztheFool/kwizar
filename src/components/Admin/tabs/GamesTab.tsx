...
import { CheckCircleIcon, NoSymbolIcon, BarsArrowUpIcon, BarsArrowDownIcon, PhotoIcon, PencilSquareIcon, ChevronDownIcon } from '@heroicons/react/24/outline';
...
export default function GamesTab() {
    ...
    // Options du combobox : tous les jeux, triés par nom (icône via gameType).
    const gameOptions = useMemo(
        () => [...games].sort((a, b) => a.label.localeCompare(b.label, 'fr')).map(g => ({ key: g.key, label: g.label, gameType: g.gameType })),
        [games],
    );
        ...
        <div id="admin-games" className="scroll-mt-24 space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-2">
                <GameCombobox
                    ...
                    onSelect={key => { const g = games.find(x => x.key === key); if (g) setSearch(g.label); }}
