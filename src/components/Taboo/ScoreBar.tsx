...
export default function ScoreBar({ scores, myTeam, currentTeam }: Props) {
    ...
    return (
        <div className="flex items-center gap-2 justify-center">
            ...
            <span className={`text-2xl font-bold leading-none transition-colors ${myActive ? (myT === '0' ? 'text-primary-500' : 'text-emerald-600') : 'text-gray-900 dark:text-white'}`}>
                {scores[myT] ?? 0}
            </span>
            <span className="text-gray-400 dark:text-gray-500 font-bold text-sm">–</span>
            <span className={`text-2xl font-bold leading-none transition-colors ${advActive ? (advT === '0' ? 'text-primary-500' : 'text-emerald-600') : 'text-gray-900 dark:text-white'}`}>
                {scores[advT] ?? 0}
            </span>
