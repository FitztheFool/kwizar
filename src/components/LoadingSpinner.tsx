...
interface LoadingSpinnerProps {
    ...
    hint?: string;
    /** Diameter of the spinner in px (default 56 = w-14). Scales the whole visual. */
    size?: number;
}

export default function LoadingSpinner({ message, fullScreen = true, hint, size = 56 }: LoadingSpinnerProps) {
    const factor = size / 56;
    const spinner = (
        <div
            className="relative"
            style={{ width: size, height: size, animation: 'ln-fade 200ms ease-out' }}
        >
            <div
                className="absolute left-1/2 top-1/2"
                style={{ width: 56, height: 56, transform: `translate(-50%, -50%) scale(${factor})`, transformOrigin: 'center' }}
            >
                {/* Outer glow ring */}
