...
function Grid({ ships, receivedShots, hitShots, showShips, onShoot, label, highlight }: GridProps) {
    ...
    const renderCell = useCallback((row: number, col: number) => {
        ...
        let borderBg = 'bg-slate-800 border-slate-600';
        if (isClickable) borderBg = 'bg-slate-800 border-slate-600 hover:bg-blue-900/40 hover:border-blue-600 cursor-crosshair transition-colors';
        if (isShot && !entry && !isHit) borderBg = 'bg-blue-600/50 border-blue-400'; // raté = eau bleue
        if (isShot && (!!entry || isHit) && !entry?.ship.sunk) borderBg = 'bg-orange-500/30 border-orange-400'; // touché = orange
        if (isHighlight) borderBg += ' ring-2 ring-white';
        ...
        if (showShips && entry) {
            ...
            borderBg = `border-slate-600 ${isHighlight ? 'ring-2 ring-white' : ''}`;
        }
        ...
        if (isShot) {
            ...
            shotOverlay = isActualHit
                ? <span className="relative z-10 text-white text-xs font-bold drop-shadow-md">✕</span>
                : <span className="relative z-10 text-blue-200 text-xs drop-shadow">●</span>;
        }
