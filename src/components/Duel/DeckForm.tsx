...
interface Props {
    ...
    /** Retourne un message d'erreur, ou null si succès (le parent gère la redirection). */
    onSubmit: (payload: DeckPayload) => Promise<string | null>;
}

function initialItems(initial?: DeckFormInitial): ItemForm[] {
    const base = (initial?.items ?? []).map(i => ({ name: i.name, imageUrl: i.imageUrl, uploading: false }));
    while (base.length < MIN_ITEMS) base.push(emptyItem());
    return base;
}
...
export default function DeckForm({ heading, submitLabel, cancelHref, initial, onSubmit }: Props) {
    ...
    const [items, setItems] = useState<ItemForm[]>(() => initialItems(initial));
    ...
    const submit = async () => {
        ...
        // succès : le parent redirige (composant démonté).
    };
