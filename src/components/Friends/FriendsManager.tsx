// src/components/Friends/FriendsManager.tsx — friends management UI (Amis / Demandes / Ajouter).
// Extracted from the old /friends page so it can live inside the unified Social view's
// detail pane. Sub-tabs stay deep-linkable via the URL query (?tab=list|pending|add).
'use client';
...
const TAB_SLUG: Record<Tab, string> = { friends: 'list', requests: 'pending', add: 'add' };
const SLUG_TAB: Record<string, Tab> = { list: 'friends', pending: 'requests', add: 'add' };

/**
 * @param onMessage Called when the user taps "message" on a friend. When provided
 * (unified Social view) it opens the thread in the detail pane; otherwise the page
 * falls back to the floating dock.
 * @param initialTab Active sub-tab slug from the URL (?tab=list|pending|add),
 * forwarded by the server page so deep links select the right tab on every navigation.
 */
export default function FriendsManager({ onMessage, initialTab }: { onMessage?: (userId: string) => void; initialTab?: string }) {
    const { refresh: refreshBadge } = useFriends();
    const { openThread } = useMessages();
    const messageFriend = onMessage ?? openThread;
    const router = useRouter();
    const pathname = usePathname();

    const [tab, setTabState] = useState<Tab>(SLUG_TAB[initialTab ?? ''] ?? 'friends');
    // Re-sync when the URL's tab changes (e.g. arriving from the command palette).
    useEffect(() => {
        setTabState(SLUG_TAB[initialTab ?? ''] ?? 'friends');
    }, [initialTab]);
    // Keep the URL query in sync so the 3 tabs are deep-linkable (?tab=amis|demandes|ajouter).
    const setTab = (t: Tab) => {
        setTabState(t);
        router.replace(`${pathname}?tab=${TAB_SLUG[t]}`, { scroll: false });
    };
    ...
    return (
        <div className="flex flex-col min-h-0 h-full">
            {/* Tabs */}
            <div className="flex gap-1 p-1 m-3 rounded-xl bg-gray-100 dark:bg-gray-800 shrink-0">
                {tabs.map(t => (
                    <button
                        key={t.id}
                        onClick={() => setTab(t.id)}
