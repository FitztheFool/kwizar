...
export function TrapPhase({ game, myId, myTeam, lobbyId, socketRef }: TrapPhaseProps) {
    ...
    const teamBorder = myTeam === 0 ? 'bg-primary-500/10 border-primary-500/20'
        : myTeam === 1 ? 'bg-emerald-500/10 border-emerald-500/20'
            ...
            {myTeam !== null && wordToPiege ? (
                <div className="mb-5 p-4 rounded-xl bg-yellow-500/10 border border-yellow-500/20">
                    <p className="text-xs text-gray-500 dark:text-white/40 uppercase tracking-widest mb-1">
                        Mot à piéger — pour l'équipe {myTeam === 0 ? <>verte <span className="inline-block w-2.5 h-2.5 rounded-full bg-emerald-600 align-middle" /></> : <>ambre <span className="inline-block w-2.5 h-2.5 rounded-full bg-primary-500 align-middle" /></>}
                    </p>
