...
type AdminTab = 'stats' | 'users' | 'quizzes' | 'categories' | 'words';
...
export default function AdminPanel() {
    ...
    const handleAddWord = async () => {
        ...
        const res = await fetch('/api/admin/words', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ word: newWord, description: newWordDesc }),
        });
    ...
    const handleSaveWord = async () => {
        ...
        const res = await fetch('/api/admin/words', {
            method: 'PATCH',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ wordId: editingWord.id, word: editingWord.word, description: editingWord.description }),
        });
        if (!res.ok) { alert((await res.json())?.error ?? 'Erreur'); return; }
        setEditingWord(null);
        await fetchWordIndex();
                    ...
                    {activeTab === 'words' && (
                        <div id="admin-words" className="scroll-mt-24 space-y-4">
                            ...
                            <div className="bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-100 dark:border-gray-800 px-4 py-3 flex flex-wrap gap-2">
                                ...
                                />
                                <button onClick={handleAddWord} className="text-xs px-3 py-1.5 bg-green-600 hover:bg-green-700 text-white rounded-lg font-semibold transition-colors shrink-0">
                                    + Ajouter
                                </button>
                            </div>
                            ...
                            <div className="bg-gray-50 dark:bg-gray-800/50 rounded-2xl border border-gray-100 dark:border-gray-800 p-4">
                                ...
                                <div className="flex flex-wrap gap-1.5">
                                    ...
                                    {wordIndex.length === 0 && (
                                        <p className="text-xs text-gray-400 dark:text-gray-500">Aucun mot enregistré.</p>
                                    )}
                                </div>
                            </div>

                            {/* Liste des mots pour la lettre sélectionnée */}
                            {selectedLetter && (
                                        ...
                                        <div key={w.id} className="flex items-center justify-between bg-white dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 px-4 py-2.5 gap-3">
                                            {editingWord?.id === w.id ? (
                                                ...
                                                <div className="flex items-center gap-3 flex-1 min-w-0">
                                                    ...
                                                    {w.description && (
                                                        <span className="text-xs text-gray-400 dark:text-gray-500 truncate">{w.description}</span>
                                                    )}
                                                </div>
                                            )}
                                            <div className="flex gap-1.5 shrink-0">
                                                {editingWord?.id === w.id ? (
                                                    <>
                                                        <button onClick={handleSaveWord} className="text-[10px] px-2 py-0.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-semibold transition-colors">Sauvegarder</button>
                                                    ...
                                                    <>
                                                        ...
                                                        <button onClick={() => handleDeleteWord(w.id, w.word)} className="text-[10px] font-semibold text-red-500 hover:text-red-700 px-2 py-0.5 border border-red-200 dark:border-red-800 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors">Supprimer</button>
                                                    </>
                                                )}
                                            </div>
                                        </div>
