...
export default function SurrenderButton({ onSurrender, disabled }: {
    ...
    // Abandonner ne fait qu'émettre l'abandon : l'état serveur pilote l'UI ensuite
    // (game-over en 1v1 ; la partie continue avec les bots en N-joueurs).
    return (
        <button
            onClick={() => {
                if (!disabled && !done && confirm('Abandonner la partie ?')) {
                    onSurrender();
                    setDone(true);
                }
            }}
        ...
        >
            ...
            <span className="hidden sm:inline">{done ? 'Abandonné' : 'Abandonner'}</span>
        </button>
    );
}
