    ...
    const sorted = Array.from(byUser.entries())
        .map(([userId, data]) => {
            ...
            const score = game === 'skyjow' || game === 'just_one' ? avgScore
                ...
                : (game === 'snake' || game === 'tetris' || game === 'pacman' || game === 'breakout' || game === 'sutom' || game === 'space_invaders' || game === '2048' || game === 'flappy_bird' || game === 'plumber' || game === 'demineur' || game === 'sudoku') ? Math.max(...data.scores)
                    : totalScore;
